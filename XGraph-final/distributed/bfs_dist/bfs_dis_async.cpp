...
void bfs_dis_async(ArgumentParser arguments)
{
    MPI_Init(NULL, NULL);
    ...
    // 图划分
    bfs_part(AllGraph, graph_cut, AllStates, states_cut, n);
	AllGraph.FreeGraph();
	...
	bfs_async(graph, states, graph_value, false, arguments.sourceNode);
	...
	while (true)
	{
		...
		bool end = bfs_async(graph, states, graph_value);
        if(end) myFlag = 0;
        else myFlag = 1;
