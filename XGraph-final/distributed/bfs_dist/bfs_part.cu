...
void bfs_part(GraphStructure graph, GraphStructure graph_cut[], GraphStates<uint> states, GraphStates<uint> states_cut[], uint n)
{
    uint nodes = graph.num_nodes;
    uint edges = graph.num_edges;
    // locality-aware chunking Gemini划分
    ...
    // locality-aware chunking useless, simple cut Gemini划分失效，简单划分
    if(flag==1)
    {
