...
void bfs_compute(GraphStructure& graph, GraphStates<uint>& states, Subgraph& subgraph, SubgraphGenerator<uint>& subgen)
{
	Timer timer;
	timer.Start();
...
bool bfs_async(GraphStructure& graph, GraphStates<uint>&states, uint graph_value[], bool hasInit, int sourceNode)
{
	if(!hasInit)
	{
	...
	bfs_compute(graph, states, subgraph, subgen);
