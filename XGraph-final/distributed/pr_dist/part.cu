...
void part(GraphStructure graph, GraphStructure graph_cut[], GraphStates<float> states, GraphStates<float> states_cut[], uint n, bool isInternalNode[], int workerID)
{
    ...
    partition_offset[n] = graph.num_nodes;
    for (uint i = partition_offset[workerID]; i < partition_offset[workerID + 1]; ++i)
    {
        isInternalNode[i] = true;
    }
}
