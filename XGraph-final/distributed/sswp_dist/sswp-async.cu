...
void sswp_compute(GraphStructure& graph, GraphStates<uint>& states, Subgraph& subgraph, SubgraphGenerator<uint>& subgen)
{
	Timer timer;
	timer.Start();
...
bool sswp_async(GraphStructure& graph, GraphStates<uint>& states, uint graph_value[], bool hasInit, int sourceNode)
{
	if (!hasInit)
	{
		...
		states.value[sourceNode] = DIST_INFINITY;
	...
	sswp_compute(graph, states, subgraph, subgen);
