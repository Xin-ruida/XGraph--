...
void sswp_dis_async(ArgumentParser arguments)
{
    MPI_Init(NULL, NULL);
    ...
    GraphStates<uint> AllStates(AllGraph.num_nodes, true, false, false, AllGraph.num_edges);
    AllStates.ReadEdgeWeight(arguments.input + "w", AllGraph.num_edges);
    ...
    states.ReadEdgeWeightFromStates(states_cut[world_rank], graph.num_edges);

    sswp_async(graph, states, graph_value, false, arguments.sourceNode);
    ...
    while (true)
    {
        MPI_Allreduce(states.value, graph_value, AllGraph.num_nodes, MPI_UNSIGNED, MPI_MAX, MPI_COMM_WORLD);
        cout << "[" << getpid() << "]: " << "communicate " << iter++ << " times" << endl;

        bool end = sswp_async(graph, states, graph_value);
        if (end) myFlag = 0;
        else myFlag = 1;
