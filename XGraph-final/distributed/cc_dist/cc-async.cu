...
void cc_compute(GraphStructure& graph, GraphStates<uint>& states, Subgraph& subgraph, SubgraphGenerator<uint>& subgen)
{
	Timer timer;
	timer.Start();
...
bool cc_async(GraphStructure& graph, GraphStates<uint>& states, uint graph_value[], bool hasInit)
{
	if (!hasInit)
	{
	...
	cc_compute(graph, states, subgraph, subgen);
